...
export const activityApi = {
  list: async (): Promise<Activity[]> => {
    const { data } = await http.get<ActivitiesBody>('/evaluation/activities');
  ...
  getById: async (id: string): Promise<Activity> => {
    const { data } = await http.get<ActivityBody>(`/evaluation/activities/${id}`);
    return data.activity;
  },
  create: async (payload: CreateActivityPayload): Promise<Activity> => {
    const { data } = await http.post<ActivityBody>('/activities', payload);
    return data.activity;
  },
  update: async (id: string, payload: UpdateActivityPayload): Promise<Activity> => {
    const { data } = await http.put<ActivityBody>(`/activities/${id}`, payload);
    return data.activity;
  },
  remove: async (id: string): Promise<Activity> => {
    const { data } = await http.delete<ActivityBody>(`/activities/${id}`);
    return data.activity;
  },
  publish: async (id: string): Promise<Activity> => {
    const { data } = await http.post<ActivityBody>(`/activities/${id}/publish`);
    return data.activity;
  },
  listSubmissions: async (id: string, params?: { groupId?: string }) => {
    ...
    const { data } = await http.get<SubmissionsBody>(`/activities/${id}/submissions${query}`);
