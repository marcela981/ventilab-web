...
// Module 03 content and navigation (direct path: barrel index.js does not export CurriculumPanel)
import CurriculumPanel from '@/features/ensenanza/shared/components/modulos/CurriculumPanel/CurriculumPanel';

// Componentes hijos
import {
  DashboardHeader
} from '@/features/ensenanza/shared/dashboard/components';
import FlashcardSystem from '@/features/ensenanza/shared/components/modulos/FlashcardSystem';

// Extracted components
import TeachingLessonView from '@/features/ensenanza/shared/components/leccion/TeachingLessonView';
import TeachingTabs from '@/features/ensenanza/shared/components/pages/TeachingTabs';
import ProgressTabSkeleton from '@/features/ensenanza/shared/dashboard/components/ProgressTabSkeleton';

// Lazy load ProgressDashboard for better performance
const ProgressTab = lazy(() => import('@/features/progress/components/ProgressTab'));
const Module3ProgressDashboard = lazy(() => import('@/features/ensenanza/shared/dashboard/components/Module3ProgressDashboard'));
const ReadinessIndicator = lazy(() => import('@/features/ensenanza/shared/dashboard/components/ReadinessIndicator'));
...
const TeachingModule = () => {
  ...
  const {
    ...
    calculateGlobalStats = null,
    levelProgress = null,
    levelProgressAggregated = null
  ...
  const handleSectionClick = useCallback(async (moduleId, lessonId = null, category = null) => {
    const router = routerRef.current; // usa ref para obtener el router más reciente sin ser dep del memo
  ...
  const prepareDashboardData = useCallback(() => {
    ...
    // Usar una fecha base consistente para SSR (evita hydration mismatch)
  ...
  // dashboardDataForTab: useMemo con SOLO dependencias primitivas para evitar infinite render loop.
  // NO incluir funciones (handleSectionClick, handleContinueLearning, calculateModuleProgress)
  // ni objetos que se recrean en cada render (router, completedLessons, allModules).
  // Usar .size y .length (primitivos) en su lugar.
  const dashboardDataForTab = useMemo(() => {
    ...
    allModules.length,
    snapshot?.overview?.completedLessons,
  ]);
      ...
      {!isTeacherMode && activeTab === 0 && (
        dashboardDataForTab ? (
          <DashboardTab
            ...
            error={null}
            onRefresh={() => { /* el memo se actualiza automáticamente */ }}
          />
        ) : (
