...
} from '@/features/progress/services/progressService.ts';
...
export function useLessonProgress({
  ...
  const initializingRef = useRef(false); // Guard against concurrent initializeProgress executions
  ...
  useEffect(() => {
    ...
    const initializeProgress = async () => {
      // [CAMBIO 2] Guard against concurrent executions (prevents ERR_INSUFFICIENT_RESOURCES)
      if (initializingRef.current) {
        console.warn('[useLessonProgress] initializeProgress already running, skipping');
        return;
      }
      ...
      try {
        ...
        try {
          // [CAMBIO 1] Try to get progress from backend; degrade gracefully on network errors
          let progress = null;
          try {
            ...
            if (isNetworkError) {
              ...
              throw fetchError; // unexpected non-network error → outer catch handles it
            }
          }

          if (progress && (progress.completed || progress.completionPercentage > 0)) {
