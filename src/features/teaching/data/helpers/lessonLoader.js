...
import { get as apiGet } from '@/shared/services/api/http';
import { BACKEND_API_URL, logEnvDiagnostics } from '@/config/env';
// =============================================================================
// Constants
// =============================================================================
...
async function loadFromApi(lessonId) {
  // Resolved backend API URL is logged on the server during SSR
  // In the browser, calls go directly to BACKEND_API_URL (/api prefix already included)
  if (typeof window === 'undefined') {
    logEnvDiagnostics('lessonLoader.loadFromApi');
  } else {
    // Safe client-side diagnostics (no env values, only resolved URL)
    console.debug('[lessonLoader] BACKEND_API_URL:', BACKEND_API_URL);
  }
