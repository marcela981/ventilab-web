...
import { configureProgressService, getProgressSummary } from '@/features/progress/services/progressService.js';
...
import { useOutboxReconciliation } from './hooks/useOutboxReconciliation';
...
import {
  ...
  convertLegacyUpdate,
} from './utils/legacyProgressHelpers';
import { inferModuleIdFromLesson } from './utils/progressHelpers';
...
export const LearningProgressProvider = ({ children }) => {
  ...
  // Refs to always access latest session/waitForToken inside stable useCallback closures
  const sessionRef = useRef(session);
  sessionRef.current = session;
  const waitForTokenRef = useRef(waitForToken);
  waitForTokenRef.current = waitForToken;
  ...
  // Load snapshot on mount and when session changes (so a restored NextAuth session
  // triggers a fresh DB fetch even if localStorage was cleared)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setProgressByModule(prev => {
      ...
      for (const lesson of lessonsWithProgress) {
        ...
        try {
          ...
          if (typeof curriculumData?.modules === 'object') {
            for (const [modId, modData] of Object.entries(curriculumData.modules)) {
              ...
              const found = lessons.some(l => l.id === lesson.lessonId);
              if (found) {
                moduleId = modId;
                lessonIdOnly = lesson.lessonId;
                break;
              }
            }
          }
        } catch (e) {
          // curriculumData not available, fall through to next strategy
        }

        // STRATEGY 2: Fallback - look in existing progressByModule
        if (!moduleId) {
        ...
        // STRATEGY 3: Fallback - if lessonId contains '/'
        if (!moduleId && lesson.lessonId.includes('/')) {
          const parts = lesson.lessonId.split('/');
          moduleId = parts.slice(0, -1).join('/');
          lessonIdOnly = parts[parts.length - 1];
        }

        // If we still don't have a moduleId, log and skip this lesson
        if (!moduleId) {
          console.warn('[LearningProgressContext] Could not determine moduleId for lesson:', lesson.lessonId, '- skipping sync');
          continue;
        }
  ...
  const moduleProgressAggregated = useMemo(() => {
    ...
    try {
      ...
      for (const [moduleId, module] of Object.entries(curriculumData.modules)) {
        ...
        // Fallback: si no hay datos de progressByModule/snapshot.lessons pero sí de overview
        if (aggregated[moduleId].progressPercent === 0 && moduleDataFromOverview[moduleId]) {
          aggregated[moduleId] = moduleDataFromOverview[moduleId];
        }
      }

      // Módulos en overview pero no en curriculumData - agregar directamente
      for (const [modId, modData] of Object.entries(moduleDataFromOverview)) {
        if (!aggregated[modId]) {
          aggregated[modId] = modData;
        }
      }
    } catch (error) {
...
export { ProgressSyncBadge } from './components/ProgressSyncBadge';
