...
import { predefinedClinicalCases, getSimulatedPatientData } from '@/features/simulador/simuladorPaciente/casosClinicos/datos/patientSimulatedData';
import { usePatientData } from '@/features/simulador/simuladorPaciente/hooks/usePatientData';
import { useSimulation } from '@/features/simulador/simuladorPaciente/hooks/useSimulation';
import { usePatientForm } from './hooks/usePatientForm';
import { PatientForm } from './componentes/PatientForm';
import { StyledCard, menuPaperSx } from './componentes/PatientForm.styles';
...
const PatientSimulator: React.FC = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    await simulation.actions.startSimulation(initialCommand as VentilatorCommand);
            ...
            <FormControl sx={{ minWidth: 280, flex: 1 }}>
              ...
              >
                ...
                {(Object.entries(predefinedClinicalCases) as [CaseKey, { title: string }][]).map(([key, c]) => (
                  <MenuItem key={String(key)} value={key}>{c.title}</MenuItem>
                ))}
              </Select>
            </FormControl>
